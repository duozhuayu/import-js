...
function importStatementGroupIndex(
  ...
  return groupingOrder[`${style} ${pathType}`];
}

function generateGroupingOrder(orderArray: Array<string>): Object {
  const groupingOrder = {};

  orderArray.forEach((group: string, index: number) => {
    groupingOrder[group] = index;
  });
...
export default class ImportStatements {
  ...
  constructor(config: Configuration, imports: Object = {}) {
    ...
    this.groupingOrder = generateGroupingOrder(this.config.get('customGroupOrder', {
      groupOrderArray: GROUPINGS_ARRAY,
    }));
  }
